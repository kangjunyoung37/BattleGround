...
public class PatrolAction : Action
{
    ...
    private void Patrol(StateController controller)
    {
        ...
        controller.nav.speed = controller.generalStates.patrolSpeed;
        if(controller.nav.remainingDistance <= controller.nav.stoppingDistance && !controller.nav.pathPending)
        {
            ...
            if (controller.variables.patrolTimer >= controller.generalStates.patrolWaitTime)
            {
        ...
        {
            Debug.LogWarning("웨이포인트가 없음 세팅해주세여", controller.gameObject);

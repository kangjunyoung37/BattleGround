...
public class SoundManager :SingletonMonobehaviour<SoundManager>
{
    ...
    public AudioSource UI_aduio = null;
    ...
    void Start()
    {
        ...
        if(fadeA_audio == null)
        {
            GameObject fadeA = new GameObject(FadeA, typeof(AudioSource));//AudioSource라는 컴포넌트가 붙어서 게임오브젝트가 생성됌
        ...
        if(UI_aduio == null)
        {
            ...
            this.UI_aduio = ui.GetComponent<AudioSource>();
            this.UI_aduio.playOnAwake = false;
        }
        ...
        if(this.mixer != null)
        {
            ...
            this.fadeB_audio.outputAudioMixerGroup = mixer.FindMatchingGroups(BGMGroupName)[0];//사운드 볼륨 조정을 위해 설정
            this.UI_aduio.outputAudioMixerGroup = mixer.FindMatchingGroups(UIGroupName)[0];
    ...
    {
        currentRatio = Mathf.Clamp01(currentRatio);//0과 1사이의 값으로 정해준다
    ...
    void PlayAduioSource(AudioSource source, SoundClip clip,float volume)
    {
        ...
        source.maxDistance = clip.maxDistane;
    ...
    public void FadeIn(SoundClip clip, float time,Interpolate.EaseType ease)
    {
        if(this.IsDiffentSound(clip))
        {
            ...
            PlayAduioSource(fadeA_audio, currentSound, 0.0f);
    ...
    public void FandeIn(int index, float time, Interpolate.EaseType ease)
    {
    ...
    public void FadeTo(SoundClip clip ,float time,Interpolate.EaseType ease)
    {
        ...
        else if(this.IsDiffentSound(clip))
        {
            ...
            if(currentPlayingType == MusicPlayingType.SourceA)
            {
                PlayAduioSource(fadeB_audio, currentSound, 0.0f);
    ...
    public void PlayBGM(SoundClip clip)
    {
        if(this.IsDiffentSound(clip))
        {
            ...
            this.lastSound = this.currentSound;
            this.currentSound = clip;
            PlayAduioSource(fadeA_audio, clip, clip.maxVolume);
            if(currentSound.HasLoop())
            {
                this.isTicking = true;
                DoCheck();
    ...
    {
        PlayAduioSource(UI_aduio, clip, clip.maxVolume);
    }
    public void PlayEffectSound(SoundClip clip)
    {
        ...
        for(int i = 0; i < this.EffectChannelCount; i++)
        {
            ...
            {
                PlayAduioSource(this.effect_audios[i], clip, clip.maxVolume);
        ...
        if(isPlaySuccess == false)
        {
            ...
            PlayAduioSource(this.effect_audios[selectIndex], clip, clip.maxVolume);
        }
    }
    ...
    public void PlayOndShotEffect(int index, Vector3 position,float volume)
    {

...
public class CoverLookUp : MonoBehaviour
{
    ...
    private List<int> coverHashCodes; //유니티 고유 아이디
    private Dictionary<float, Vector3> fillteredSpots; //필터링된 지점들을 모아놓은 곳
    ...
    private Vector3[] GetSpots(GameObject go, LayerMask obstacleMask)
    {
        ...
        foreach(Collider col in go.GetComponents<Collider>())
        {
            ...
            float range = 2 * col.bounds.extents.y;//크기
    ...
    //목표물이 경로에 있는지 확인, 대상이 각도 안에 있고 지점보다 가까이 있는지 확인

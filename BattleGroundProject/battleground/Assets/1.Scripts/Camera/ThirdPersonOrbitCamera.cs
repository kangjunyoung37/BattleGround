...
//충돌체크 : 이중 출동 체크 기능 (캐릭터로부터 카메라, 카메라로부터 캐릭터사이)
//사격 반동을 위한 기능
//FOV 변경 기능
...
public class ThirdPersonOrbitCamera : MonoBehaviour
{
    ...
    private Vector3 smoothPivotOffset;//카메라 피봇용 보관용 벡터
    private Vector3 smoothCamOffset;//카메라 위치용 보관용 벡터
    private Vector3 targetPivotOffset; //카메라 피봇용 보관용 벡터
    private Vector3 targetCamOffset;// 카메라 위치용 보관용 벡터
    ...
    public void ResetMaxVerticalAngel()
    {
    ...
    bool DoubleViewingPosCheck(Vector3 checkPos,float offset)//더블체크
    {
    ...
    {
        //마우스 이동 값
        angleH += Mathf.Clamp(Input.GetAxis("Mouse X"), -1f, 1f) * horizontalAimingSpeed;//마우스 이동속도를 곱해줌
        ...
        Vector3 noCollisionOffset = targetCamOffset; // 조준할 때 카메라의 오프셋값, 조준할때와 평소가 다르다.
